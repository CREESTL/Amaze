...
describe("Core", () => {
  ...
  describe("Modifiers", () => {
    ...
    it("Allow check blacklisted when contract is paused", async () => {
      ...
      expect(
        await core.connect(ownerAcc).checkBlacklisted(clientAcc1.address)
      ).to.equal(true);
    });
  });
  ...
  describe("Getters", () => {
    it("Should check that user is in blacklist", async () => {
      ...
      await expect(await core.checkBlacklisted(clientAcc1.address)).to.equal(
        false
      );
  ...
  describe("Setters", () => {
    it("Set a new Maze address", async () => {
      ...
      await expect(core.setMaze(clientAcc1.address)).to.emit(
        core,
        "MazeChanged"
      );
    ...
    it("Set a new Vesting address", async () => {
      ...
      await expect(core.setVesting(clientAcc1.address)).to.emit(
        core,
        "VestingChanged"
      );
    ...
    it("Set a new Farming address", async () => {
      ...
      await expect(core.setFarming(clientAcc1.address)).to.emit(
        core,
        "FarmingChanged"
      );
