    ...
    describe("Deployment", () => {
        ...
        describe("Fails", () => {
            it("Should fail to deploy with zero core address", async () => {
                let mazeFactory = await ethers.getContractFactory("Maze");
                await expect(
                    mazeFactory.deploy(zeroAddress)
                ).to.be.revertedWith("Maze: Core cannot have zero address");
            });
        });
    });
    ...
    describe("Modifiers", () => {
        ...
        describe("Paused", () => {
            it("Should forbid any operations when contract is paused", async () => {
                ...
                await expect(
                    maze.connect(ownerAcc).burn(burnAmount)
        ...
        describe("Balances", () => {
            describe("Included users", () => {
                ...
                it("Should return balance of the user", async () => {
                    ...
                    let ownerStartBalance = await maze.balanceOf(
                        ownerAcc.address
                    );
                    ...
                    let ownerEndBalance = await maze.balanceOf(
                        ownerAcc.address
                    );
                    let clientEndBalance = await maze.balanceOf(
                        clientAcc1.address
                    );
                    ...
                    expect(clientEndBalance).to.be.gt(
            ...
            describe("Excluded users", () => {
                ...
                it("Should return balance of the user", async () => {
                    ...
                    await maze
                        .connect(ownerAcc)
                        .excludeFromStakers(ownerAcc.address);
                    await maze
                        .connect(ownerAcc)
    ...
    describe("Main functions", () => {
        ...
        describe("Burn", () => {
            describe("From included accounts", () => {
                it("Should burn tokens of the user", async () => {
                    ...
                    expect(ownerEndBalance).to.equal(
                        ownerStartBalance.sub(burnAmount)
                    );
                    expect(endTotalSupply).to.equal(
                        startTotalSupply.sub(burnAmount)
                    );
            ...
            describe("Fails", () => {
                ...
                it("Should fail to transfer if not enough tokens for transfer and fee for both included", async () => {
                    ...
                    await expect(
                        maze
                            .connect(clientAcc1)
                            .transfer(ownerAcc.address, transferAmount)
                    ).to.be.revertedWith(
        ...
        describe("TransferFrom", () => {
            it("Should decrease allowance after transfer", async () => {
                ...
                await maze
                    .connect(ownerAcc)
                    .excludeFromStakers(clientAcc2.address);
                ...
                let startAllowance = await maze.allowance(
                    ownerAcc.address,
                    clientAcc1.address
                );

                let client2StartBalance = await maze.balanceOf(
                    clientAcc2.address
                );
                await maze
                    .connect(clientAcc1)
                ...
                let client2EndBalance = await maze.balanceOf(
                    clientAcc2.address
                );
                expect(client2EndBalance).to.equal(
                    client2StartBalance.add(transferAmount)
                );

                let endAllowance = await maze.allowance(
                    ownerAcc.address,
                    clientAcc1.address
                );
                expect(endAllowance).to.equal(
                    startAllowance.sub(transferAmount)
                );
            });
        });
        ...
        describe("Set fees", () => {
            it("Should set new fee percentage", async () => {
                ...
                await expect(maze.connect(ownerAcc).setFees(100)).to.emit(
                    maze,
                    "SetFees"
                );
            ...
            describe("Fails", () => {
                it("Should fail to set to high fee", async () => {
                    ...
                    await expect(
                        maze
                            .connect(ownerAcc)
                            .setFees(BigNumber.from("1000000000"))
        ...
        describe("Whitelist", () => {
            describe("Add to whitelist", () => {
                it("Should add to whitelist", async () => {
                    ...
                    expect(
                        await maze.isWhitelisted(clientAcc1.address)
                    ).to.equal(false);

                    await expect(
                        maze
                            .connect(ownerAcc)
                            .addToWhitelist(clientAcc1.address)
            ...
            describe("Remove from whitelist", () => {
                it("Should remove from whitelist", async () => {
                    ...
                    await expect(
                        maze
                            .connect(ownerAcc)
                            .removeFromWhitelist(clientAcc1.address)
        ...
        describe("Stakers", () => {
            describe("Exclude from stakers", () => {
                it("Should exclude account from stakers", async () => {
                    ...
                    expect(await maze.isExcluded(clientAcc1.address)).to.equal(
                        false
                    );
